...
static long		mwStatMaxNumAlloc = 0L;
...
void* mwMalloc( size_t size, const char* file, int line) {
    ...
    mwNumCurAlloc ++;
	if (mwNumCurAlloc > mwStatMaxNumAlloc)
		mwStatMaxNumAlloc = mwNumCurAlloc;
...
static void mwStatReport()
{
    ...
    mwWrite( " M)ost allocations made     : %lu\n", mwStatMaxNumAlloc);
    mwWrite( " N)umber of allocations made: %lu\n", mwStatNumAlloc );
    mwWrite( " L)argest memory usage      : %lu\n", mwStatMaxAlloc );
    mwWrite( " T)otal of all alloc() calls: %lu\n", mwStatTotAlloc );
    mwWrite( " U)nfreed bytes totals      : %lu\n", mwStatCurAlloc );
    FLUSH();
