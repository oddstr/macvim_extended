...
(export
  '(; These functions are implemented in if_ecl.c and probably use "internal"
    ...
    append argc argidx argv browse bufexists buflisted bufloaded bufname bufnr
    bufwinnr num-buffers byte2line char2nr cindent col confirm
    cursor
    expand
    getcwd
    getline
...
(defun eval-range ()
  ...
  (with-input-from-string (s (format nil
                                     ...
                                     (vim:buffer-lines :buffer (vim:current-buffer)
                                                       ;; ranges for Vim buffers start at 1, 
                                                       ;; whereas buffer-lines expects 0..lines-in-buffer
						       :start (1- (car vim:range))
						       :end (cdr vim:range))))
